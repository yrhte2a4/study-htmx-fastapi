...
app = FastAPI(title="OpenAI Agent SDK MCPエージェント")
...
templates.env.globals["moment"] = lambda: type('obj', (object,), {'format': lambda self, fmt: datetime.now().strftime(fmt.replace('HH:mm', '%H:%M'))})()
...
async def read_root(request: Request):
    ...
    # available_toolsが空の場合のフォールバック
    tools_list = []
    if available_tools:
        tools_list = [{"name": tool.name, "description": tool.description} for tool in available_tools]
...
async def chat(request: Request):
    ...
    try:
        ...
        if not agent:
            return templates.TemplateResponse("error_response.html", {
                "request": request,
                "error": "エージェントが初期化されていません。アプリケーションを再起動してください。"
            })
    
    except Exception as e:
        return templates.TemplateResponse("error_response.html", {
            "request": request,
            "error": f"リクエスト処理エラー: {str(e)}"
        })
    ...
    if hasattr(result, 'raw_responses'):
        for raw_response in result.raw_responses:
            if hasattr(raw_response, 'usage'):
                ...
                usage_info["input"] = getattr(usage, 'input_tokens', 0)
                usage_info["output"] = getattr(usage, 'output_tokens', 0)
                usage_info["total"] = getattr(usage, 'total_tokens', 0)
